...
void write_png(const char* filename, const int width, const int height, const color* image) {
    ...
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            ...
            const int src_index = (height - 1 - j) * width + i;
            const int dst_index = (j * width + i) * 3;
    ...
    // Write PNG file
    const int result = stbi_write_png(filename, width, height, 3, rgb_data.data(), width * 3);
...
void write_image(const char* filename, int width, int height, const color* image) {
