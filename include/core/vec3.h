...
public:
    ...
    __host__ __device__ static vec3 random() {
        return vec3(0.5f, 0.5f, 0.5f); // Placeholder for host compilation
    }

    __host__ __device__ static vec3 random(float min, float max) {
        return vec3((min+max)/2, (min+max)/2, (min+max)/2); // Placeholder for host compilation
    }
